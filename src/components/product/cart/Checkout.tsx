...
const StripeCheckoutButton:React.FC<Props> = ({ products }) => {
    ...
    return (
      <>
      {
        loading && (
          <Button className="py-3 text-sm text-white uppercase w-full"  disabled>
            <Loader2 className="animate-spin mr-4" size={16} />Please wait
          </Button>
        )
      }
      ...
      {
        !loading && (
          <Button onClick={()=>handleStripeCheckout()} className="py-3 text-sm text-white uppercase w-full">Checkout</Button>
        )
      }
